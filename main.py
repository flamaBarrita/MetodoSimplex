...
def metodo_simplex(variables,restricciones): 
    ...
    for i in range(1, len(laterales)):  # i recorre las filas de s empezando de la 2 o indice 1
        ...
        print(f"\nFila '{laterales[i]}':")
        for z in range(len(encabezados)):
            
            if z < variables:  #se agregan los coeficientes de las restricciones
                # BLOQUE DE ROBUSTEZ -> Coeficientes de las restricciones (a1, a2, ...)
                while True:
                    try:
                        valor = float(input(f"  {encabezados[z]} = "))
                        fila.append(valor)
                        break # Sale del bucle while si la entrada es válida
                    except ValueError:
                        print("Entrada no válida, ingresa un número.")
                    
            elif z == len(encabezados) - 1:  # columna de solución
            ...
            elif z >= variables and z < len(encabezados) - 1: # columnas de holgura (s1, s2, ...)
                if z == diagonal:  # Si la columna actual j es la columna de su propia variable de holgura
    ...
    for z in range(len(laterales)):
        print(f"{laterales[z]:<5} |", end=" ")
        for valor in tabla[z]:
    ...
    while True:
        ...
        for z in range(1, num_filas): # Comienza en la fila 1 (excluye la fila Z)
            temp_renglon = tabla[z][col_pivote]
            val_solucion = tabla[z][-1]
    ...
    for z in range(1, len(laterales)):
        var_nombre = laterales[z]
...
def metodo_grafico(num_restricciones):
    ...
    for z in range(num_restr):
        for j in range(z + 1, num_restr):
            
            # Extraer coeficientes para formar la matriz del sistema de ecuaciones
            a1_i, a2_i, b_i = datos_restr[z]
...
while True:
    try:
        print("PROGRAMACIÓN LINEAL UTILIZANDO EL MÉTODO SIMPLEX EN PYTHON")
